...
class alu(Module):
    ...
    def __init__(self):
        ...
        self.comb += Case(self.operation,{
            0b0000000000000001: self.alu_out.eq(self.operand1 + self.operand2), # ADD 0x1 #Fixme signed calculations not properly working
...
class cpuModule(Module):
    def __init__(self):
        ...
        #PC Increase 
        self.sync += program_counter.eq(program_counter + 1) # Increase the program counter
        ...
        #Fee to ALU
        self.comb += [
            If(mux1_select, alu1.operand2.eq(Cat(instruction[20:32],Replicate(instruction[31],20)))).Else(alu1.operand2.eq(registers.reg_data2_out)),
        ...
        self.comb += [If(opcode==0b0110011,mux1_select.eq(0), Case(funct3,alu_operations), registers.write.eq(1)).
                    Elif(opcode==0b0010011,mux1_select.eq(1), Case(funct3,alu_operations), registers.write.eq(1)),
                    ]
